...
fn components_from_el(
    ...
    let orb_a = - smbh_mass/(2.0*e);

    let mut one_minus_ecc2_sqrt = l/(smbh_mass * orb_a).sqrt();
...
pub fn transition_physical_as_el(
    ...
    let omega_0 = smbh_mass.powi(2) / ell0.powi(3);
    let eps0 = ell0 * omega_0;

    let base = -2.0 * eps2;
    let omega_2 = base.powf(1.5) / smbh_mass;
    ...
    if delta_e * (e2 - e1) < 0.0 {
        ...
        for r in roots_array.iter().take(roots_count) {
            let ell = r * ell0;
            let omega = smbh_mass.powi(2) / ell.powi(3);
...
pub fn encounters_new_orba_ecc_helper(
    smbh_mass: f64,
    orb_a_give: f64,
    orb_a_take: f64,
    mass_give: f64,
    ...
    ecc_give: f64,
    ecc_take: f64,
    radius_give: Option<f64>,
    radius_take: f64,
    id_num_give: u32,
    id_num_take: u32,
    delta_energy_strong: f64,
    flag_obj_types: i32, // most of it not used
    ...
    let (v_relative, v_esc_sq) = match flag_obj_types {
        0 => {
            let radius_give_geometric = radius_give.unwrap() * 2.0 * smbh_mass_geometric;
    ...
    // at present don't use one of the arguments, commenting out
    ...
    let (e_give_final, e_take_final, j_give_final, j_take_final) = transition_physical_as_el(e_give_initial, j_give_initial, e_take_initial, j_take_initial, delta_e, mass_give_geometric, mass_take_geometric, Some(smbh_mass_geometric));
    ...
    let (orb_a_give_final, orb_a_take_final, ecc_give_final, ecc_take_final, id_num_unbound) = if e_give_initial + delta_e > 0.0 {
        ...
        // removing unnecessary None argument
        let (orb_a_take_final, ecc_take_final) = components_from_el(e_take_final / mass_take_geometric, j_take_final / mass_take_geometric, smbh_mass_geometric);
        ...
        let (orb_a_give_final, ecc_give_final) = components_from_el(e_give_final / mass_give_geometric, j_give_final / mass_give_geometric, smbh_mass_geometric);
